...
def init_connection(client_id, client_secret):
    ...
    client.fetch_token('https://us.battle.net/oauth/token', grant_type='client_credentials')
    return client
    ...
    for auction in auctions:
        ...
        if (id == 168583 or id == 170554):
            ...
            if type(price) is int and type(quantity) is int:
                ...
                if id not in items:
                    items[id] = { 'id': id, 'price': price, 'quantity': quantity }
                else:
                    items[id]['quantity'] += quantity
                    existing_price = items[id]['price']
                    if price < existing_price:
                        items[id]['price'] = price
    listings = [(item['id'], item['price'], item['quantity'], datetime.datetime.utcnow()) for (k, item) in items.items()]
    db.executemany('INSERT INTO auctions VALUES (?, ?, ?, ?)', listings)
    db.commit()
    db.close()

def add_client_arguments(subparser):
    subparser.add_argument('--client-id', required=True, type=str, help='battle.net API client id')
    subparser.add_argument('--client-secret', required=True, type=str, help='battle.net API client secret')

def main():
    parser = argparse.ArgumentParser()
    subparsers = parser.add_subparsers()

    parser_download = subparsers.add_parser('download', help='download the latest data')
